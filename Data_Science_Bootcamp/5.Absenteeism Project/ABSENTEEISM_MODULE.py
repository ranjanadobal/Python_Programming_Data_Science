...
# At the beginning of the file, we import all the required libraries. Then we define the custom scaler class.
# Finally, we've got the absenteeism model class. That's the only new chunk of code. 
# It consists of five methods: load and clean data, predicted probability, predicted output category and predicted outputs. in it is the constructor. When we create an instance of a class, it initializes the attributes of that class. In this case, to create an instance of absenteeism model class, there are two required arguments. The model file and the scaler file we've created. If they're correctly specified, init will unpickle them and load the model information into the variable reg and the scaler file into the variable scaler.
# 
# Load and clean data takes a CSV as an argument and pre-processes the data in the same way we did for the training part. We pass the data through this method to pre-process it.
# 
# We'll save this notebook as a .py file. This'll be called a module. We'll import and use the functions written in this module in a new notebook file and with new data.
...
class absenteeism_model():
        ...
        # take a data file (*.csv) and preprocess it in the same way as earlier
